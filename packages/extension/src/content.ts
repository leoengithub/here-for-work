...
async function applyPlan(plan: FillPlan) {
  return applyFillPlan(plan, activeFingerprint);
}
...
chrome.runtime.onMessage.addListener((message, _sender, respond) => {
  ...
  if (message.type === "fill_plan") {
    void applyPlan(message.plan).then((results) => respond({ ok: true, results })).catch((error) => {
      respond({ ok: false, error: error instanceof Error ? error.message : String(error) });
    });
    return true;
  }
