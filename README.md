...
Lever still require separate live no-finalization observations before they can be treated
as equally proven.
...
- The browser extension is the primary form driver, not the primary product interface. Success requires exactly one result for every planned field and a correct settled read-back for every required fillable value. Expired transport or handshake, a wrong or missing tab, zero compatible fields, missing results, read-back mismatch, and unsupported multi-page, modal, or iframe flows are driver failures that become human-visible recovery states unless a separately supported review-only fallback contract exists.
- Only one browser driver owns a preparation at a time. The extension may release its lease only to a separately supported review-only fallback contract. The currently audited career-ops `apply` / Playwright surfaces do not expose the required public lease and typed result boundary, so HereForWork must keep that fallback unavailable for now. If autofill cannot proceed safely, HereForWork preserves an ordered copy/paste recovery path. No driver may submit.
