...
architecture with secondary Settings controls. Settings owns configuration, diagnostics, and user-initiated maintenance/lifecycle utilities; operational attention and recovery remain in Queue. This package was rebuilt after the typed
...
- Preparation provider, background checks, diagnostics, and Queue filters live in Settings. Settings also contains user-initiated maintenance/lifecycle utilities such as backup, export, tests, and quit; it never resolves operational attention. Actionable role attention and recovery stay in Queue. Filters start from verified career-ops profile preferences and apply to current unprepared roles and future imports.
