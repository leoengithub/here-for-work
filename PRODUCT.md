...
Discovery must exclude roles classified as suspicious before they reach Queue; users are not expected to investigate scams or employer identity as queue work. Prepare keeps a suspicious result only as a live-change safety backstop. It is otherwise gated by the typed career-ops evaluation before report/CV artifacts are committed: viable matches and `Proceed with Caution` legitimacy results continue automatically with their concrete warning preserved. A confirmed authorization conflict, a suspicious live result, or a below-threshold verified match stops the automatic path; missing or inconclusive authorization evidence does not. Undo preparation means the role is unsuitable: record Discarded, remove only its generated preparation artifacts, and clear the local preparation/browser state.
