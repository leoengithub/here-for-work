...
- A complete career-ops report for every valid evaluation and score-gated career-ops CV/PDF generation through the supported `auto_pdf_score_threshold`, with HereForWork reading the effective configured value rather than substituting one. The approved target and current personal configured value are `3.5`; if the key is absent, upstream falls back to `3.0`.
...
3. Every valid evaluation writes a complete career-ops report. career-ops generates its CV/PDF when its supported `auto_pdf_score_threshold` allows it; HereForWork may read and validate the setting but never silently changes it. The approved target and current personal configured value are `3.5`; if the key is absent, upstream falls back to `3.0`. HereForWork then adds every viable, non-suspicious evaluated role to the ordered queue.
...
Only one driver may own a preparation. The extension releases its lease before any review-only fallback driver may take ownership. Automatic fallback is allowed only after a hard pre-fill failure without authentication, CAPTCHA, anti-bot, partial-fill, or uncertain state; those conditions require visible human handoff. The audited career-ops `apply` surfaces are not yet a supported HereForWork fallback because they do not expose a transferable lease or a stable public result contract. The approved target remains a review-only fallback boundary, potentially backed by career-ops in the future only if that boundary becomes callable without internal imports or HereForWork-specific entry points. If safe autofill still cannot complete, the grounded ordered answer plan remains available for copy/paste. Neither driver has a submission operation.
